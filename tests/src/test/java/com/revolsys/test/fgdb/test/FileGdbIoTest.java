package com.revolsys.test.fgdb.test;
...
public class FileGdbIoTest {
  ...
  public static void doWriteReadTest(GeometryFactory geometryFactory, final DataType dataType,
    ...
    if (geometry instanceof Polygonal) {
      name += "_" + geometry.getGeometryComponents(LinearRing.class)
        .size();
    }
    ...
    try (
      ...
      final List<Record> objects = recordStore.newQuery(typePath)
        .getRecords();
      Assert.assertEquals("Geometry Count", 1, objects.size());
      final Geometry actual = objects.get(0)
        .getGeometry();
