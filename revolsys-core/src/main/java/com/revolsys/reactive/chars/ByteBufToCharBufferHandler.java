...
public class ByteBufToCharBufferHandler implements Subscriber<ByteBuf> {
  ...
  public ByteBufToCharBufferHandler(Publisher<ByteBuf> source, final FluxSink<CharBuffer> sink,
    Charset charset) {
    this(source, sink, charset, 8192);
  }

  public ByteBufToCharBufferHandler(Publisher<ByteBuf> source, final FluxSink<CharBuffer> sink,
    Charset charset, int bufferSize) {
  ...
  private synchronized void emit() {
    ...
    do {
      if (this.state.compareAndSet(SourceSubscriberState.FULL, SourceSubscriberState.PEEK)) {
        final ByteBuf buffer = this.currentValue;
        final int readableBytes = buffer.readableBytes();
        if (readableBytes > 0) {
          final int count = Math.min(readableBytes, this.bufferSize);
  ...
  public void onError(final Throwable t) {
    ByteBufToCharBufferHandler.this.sink.error(t);
  }

  @Override
  public void onNext(final ByteBuf value) {
