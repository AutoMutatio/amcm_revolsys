...
public interface BaseCloseable extends Closeable {
  ...
  static <R extends AutoCloseable, V> Flux<V> fluxUsing(final Callable<R> supplier,
    final Function<? super R, ? extends Publisher<V>> action) {
    return Flux.using(supplier, action, closer());
  }
